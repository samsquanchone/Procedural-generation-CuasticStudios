...
public class PortalGeneration : MonoBehaviour
{
    [SerializeField]
      ...
      public void GeneratePortal(int levelDepth, int levelWidth, float distanceBetweenVertices, LevelData levelData)
      {
       //Generate enemyMap using Perlin noise
       float[,] portalMap = this.noiseMapGeneration.GeneratePerlinNoiseMap (levelDepth, levelWidth, this.levelScale, 0, 0, this.waves);
        ...
        for (int xIndex = 0; xIndex < levelWidth; xIndex++) {
            ...
            if (terrainType.name != "water") {
                float portalValue = portalMap [zIndex, xIndex];
                ...
                for (int neighborZ = neighborZBegin; neighborZ <= neighborZEnd; neighborZ++) {
                    for (int neighborX = neighborXBegin; neighborX <= neighborXEnd; neighborX++) {
                        float neighborValue = portalMap [neighborZ, neighborX];
                        ...
                        if (neighborValue >= maxValue) {
                            ...
                            // if the current portal noise value is the maximum one, place a enemy in this location
                            if (portalValue == maxValue) {
                                Vector3 portalPosition = new Vector3(xIndex*distanceBetweenVertices, meshVertices[vertexIndex].y, zIndex*distanceBetweenVertices);
                                GameObject portal = Instantiate (this.portalPrefab[biome.index], portalPosition, Quaternion.identity) as GameObject;
